...
export class UploadController {
        ...
        try {
            ...
            if (contentType.includes('multipart/form-data')) {
                ...
                if (!body.file || !body.fileName || !body.userId) {
                    return c.json({ success: false, message: 'file, fileName, and userId are required' }, 400);
                }
                file = body.file;
                ...
                return c.json({ success: false, message: 'Unsupported content-type. Use multipart/form-data or application/json' }, 415);
            }
            // Validate file type
            const allowedTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/webp', 'image/avif'];
            const normalizedType = fileType?.toLowerCase();
            if (normalizedType && !allowedTypes.includes(normalizedType)) {
                return c.json({ success: false, message: `Invalid file type: ${fileType}. Allowed: JPEG, PNG, WEBP, AVIF` }, 400);
            }
    ...
    deleteScreenshot = async (c) => {
        try {
            ...
            console.error('[PaymentUploadController] Error:', error);
            return c.json({ success: false, message: error.message || 'Failed to delete payment screenshot' }, 500);
        }
    };
}
