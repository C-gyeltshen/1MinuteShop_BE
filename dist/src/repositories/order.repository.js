...
export class OrderRepository {
     ...
     */
    async generateOrderNumber(storeOwnerId) {
     ...
     */
    async findByOrderNumber(storeOwnerId, orderNumber) {
        return await prisma.order.findUnique({
            where: {
     ...
     */
    async updatePaymentStatus(orderId, paymentStatus, paymentScreenshotUrl) {
        return await prisma.order.update({
            where: { id: orderId },
            data: {
                paymentStatus,
    ...
    async findAll(params) {
        const { storeOwnerId, customerId, orderStatus, paymentStatus, startDate, endDate, page = 1, limit = 10, search = '', sortBy = 'createdAt', sortOrder = 'desc', } = params;
     ...
     */
    async findByStoreOwner(storeOwnerId, page = 1, limit = 10) {
        const skip = (page - 1) * limit;
        const [orders, total] = await Promise.all([
            prisma.order.findMany({
                where: { storeOwnerId },
     ...
     */
    async getStoreStatistics(storeOwnerId) {
        const [totalOrders, ordersByStatus, ordersByPaymentStatus, recentOrders,] = await Promise.all([
     ...
     */
    async storeOwnerExists(storeOwnerId) {
