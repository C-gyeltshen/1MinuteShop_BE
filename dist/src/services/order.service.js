...
export class OrderService {
        ...
        try {
            ...
            if (!customer) {
                ...
                if (!data.customerName || !data.email || !data.phoneNumber) {
                    throw {
                        statusCode: 400,
                        message: "Customer not found and customer details not provided for creation",
                    };
                }
                // Create customer
    ...
    async getOrdersByStoreOwnerId(storeOwnerId) {
        try {
    ...
    async updateOrderStatus(orderId, data) {
        try {
            ...
            // Update order status
            const updatedOrder = await orderRepository.updateOrderStatus(orderId, data.orderStatus);
            return {
                statusCode: 200,
    ...
    async updatePaymentStatus(orderId, data) {
        try {
            ...
            const updatedOrder = await orderRepository.updatePaymentStatus(orderId, data.paymentStatus);
            return {
                statusCode: 200,
