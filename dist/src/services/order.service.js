...
export class OrderService {
    ...
    async createOrder(data) {
        ...
        for (const item of data.items) {
            const validation = await orderRepository.validateProduct(item.productId, item.quantity);
     ...
     */
    async getOrderByNumber(storeOwnerId, orderNumber) {
     ...
     */
    async getStoreOrders(storeOwnerId, page = 1, limit = 10) {
    ...
    async updatePaymentStatus(orderId, paymentStatus, paymentScreenshotUrl) {
        ...
        const updatedOrder = await orderRepository.updatePaymentStatus(orderId, paymentStatus, paymentScreenshotUrl);
        return {
            success: true,
