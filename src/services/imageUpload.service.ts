...
import type { UploadImageDTO, UploadPaymentScreenshotDTO } from "../types/imageUpload.types.js";
...
export class ImageUploadService {
  private uploadRepository: ImageUploadRepository;

  constructor() {
    this.uploadRepository = new ImageUploadRepository();
  }

  async uploadProductImage(data: UploadImageDTO) {
    try {
      ...
      if (normalizedFileType && !allowedTypes.includes(normalizedFileType)) {
        throw new Error(
          `Invalid file type: ${data.fileType}. Allowed types: JPEG, PNG, GIF, WEBP, AVIF`,
        );
      }
   ...
   */
  async updateProductImage(oldPath: string, newImageData: UploadImageDTO) {
    try {
  ...
  async uploadPaymentScreenshot(data: UploadPaymentScreenshotDTO) {
    try {
      ...
      if (normalizedFileType && !ALLOWED_TYPES.includes(normalizedFileType)) {
        throw new Error(
          `Invalid file type: ${data.fileType}. Allowed types: JPEG, PNG, WEBP, AVIF`,
        );
      }
